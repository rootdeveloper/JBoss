 ...
 */
function focusTab() {
	jQuery('input, img, select').live("keydown", function(e) {
		
		var key = e.charCode ? e.charCode : e.keyCode ? e.keyCode : 0;

		
		if (key == 9) {  // 9 para TAB
			var inputs =  jQuery(this).parents("form").eq(0).find(":input:text:visible:enabled:not([readonly='readonly']), :input:checkbox:visible:enabled, :input:password:visible:enabled, .BotaoImagemSemMarginTop, .ComboField:visible:enabled, .TabWalk:visible:enabled, .SelectMultiple:visible:enabled");
