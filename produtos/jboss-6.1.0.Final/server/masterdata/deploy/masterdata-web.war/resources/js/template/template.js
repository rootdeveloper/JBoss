...
/**
 * Solução para o evento paste ser reconhecido como um keydown
 */
// Comentei pq deveria estar sendo utilizado apenas para testes (validar com Ohta / Daniels) 
//j(':input').live('input paste', function(e) {
//	j(this).keydown();
//});
...
//		j(this).children().html(j(this).children().attr('title'));  // ok, semore retorna um elemento que eh o span
//		var hasChange = false;
...
function disableInputText() {
	jQuery(":text, textarea, :checkbox, .ComboField").attr('disabled', 'disabled');
}
...
function focusTab() {
	jQuery('input, img, select, textarea').live("keydown", function(e) {
		...
		if (key == 9) {  // 9 para TAB
			var inputs =  jQuery(this).parents("form").eq(0).find(":input:text:visible:enabled:not([readonly='readonly']), :input:checkbox:visible:enabled, .BotaoImagemSemMarginTop:visible, .ComboField:visible:enabled, .TabWalk:visible:enabled");
